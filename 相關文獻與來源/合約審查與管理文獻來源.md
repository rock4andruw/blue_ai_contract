...
### 6. MAS 多代理人系統與 Sycophancy 防制（本專案 Phase 1.5 學術依據）

| 論文 | 核心發現 | arXiv |
| :--- | :--- | :--- |
| *Can LLM Agents Really Debate?* | 同質 Agent 辯論容易形成 Echo Chamber，強化錯誤而非修正 | [2511.07784](https://arxiv.org/abs/2511.07784) |
| *Talk Isn't Always Cheap: Failure Modes in Multi-Agent Debate* | 少數 Agent 在多數壓力下傾向放棄正確答案（從眾效應） | [2509.05396](https://arxiv.org/abs/2509.05396) |
| *Multi-LLM Debate: Framework, Principals, and Interventions* | 盲評（不告知他方答案）能提升 Agent 獨立性；異質模型是結構性解法 | [OpenReview](https://openreview.net/forum?id=sy7eSEXdPC) |
| *When Truth Is Overridden: Sycophancy in LLMs* | Prompt 暗示答案會觸發 Sycophancy，準確率下降；與錨定偏誤直接相關 | [2508.02087](https://arxiv.org/abs/2508.02087) |
| *Justice or Prejudice? Quantifying Biases in LLM-as-a-Judge* | 同模型系列互評存在 Self-enhancement Bias，多模型取平均更可靠 | [2410.02736](https://arxiv.org/abs/2410.02736) |
| *Judging with Many Minds: Bias in Multi-Agent LLM-as-Judge* | 增加 Agent 數量不一定減少偏見；結構性多樣性比數量更重要 | [2505.19477](https://arxiv.org/abs/2505.19477) |

**本專案對應設計**：

* 盲評（*When Truth Is Overridden* 依據）：Agent A/B 不知道 Rule Engine 答案，也不知道對方結論
* Judge 矩陣而非辯論（*Judging with Many Minds* 依據）：避免 Debate 的從眾失效模式
* Phase 2 異質模型（*Multi-LLM Debate* 依據）：Gemini Agent A + Claude Agent B，消除 Echo Chamber

### 7. 低資源法律 NLP 資料合成方法論（因應 CUAD 語言落差）

| 論文 | 核心發現 | arXiv |
| :--- | :--- | :--- |
| *DALE: Generative Data Augmentation for Low-Resource Legal NLP* | 法律文件模板化程度高、標注資料稀缺是全球共通問題；提出基於選擇性遮罩的 Encoder-Decoder 生成方法產生合成法律文件 | [2310.15799](https://arxiv.org/abs/2310.15799) |
| *DALDALL: Data Augmentation for Lexical and Semantic Diverse in Legal Domain by leveraging LLM-Persona* | 以律師、檢察官、法官等專業 persona 提示 LLM，生成語彙與語意更多元的合成法律文本，優於單純改寫 | [2603.22765](https://arxiv.org/abs/2603.22765) |

**問題背景**：原資料集計畫擬用 CUAD（30–40 筆，*arXiv:2103.06268*）補足樣本數，但 CUAD 為美國英文商業合約，與本研究「繁體中文合約風險偵測」的題目範圍直接衝突——若近半數評測樣本是英文合約，等於自我推翻「填補台灣本地化研究缺口」的核心論點，且 CUAD 的美式契約風險模式也不符合台灣業界慣例。

**本專案對應設計**：捨棄 CUAD 作為評測樣本來源，改以真實合約模板（現有 SLA/NDA v1/v2）為基礎，透過 LLM 生成更多繁體中文合約變體以補足至 80–100 筆——此作法即 DALE／DALDALL 所述的「LLM-based synthetic data generation for low-resource legal NLP」，在方法論上有國際文獻支撐，且維持資料集全程繁體中文、與研究題目一致。CUAD 保留作為 Clause Extraction 41 種條款類型的分類體系參考（見四之 2），不再作為評測資料本體。
...
| **MAS 在法律領域** | *When Truth Is Overridden (2025)* 提出 Sycophancy 問題，但無法律合約場景的實作驗證 | **本專案可主張的新貢獻：盲評 MAS 防止 Sycophancy 的法律 AI 實作** |
| **AI 不確定性揭露** | 現有合約 AI 工具（LawGeex、LegalOn）均給出確定答案，無 Pending 機制 | **本專案的差異化：AI 主動表達不確定，改善人機協作品質** |
...
| 功能 | 本系統 | Lumine AI | LawGeex / LegalOn |
| :--- | :--- | :--- | :--- |
| 差異比對 | ✅ | ✅ | ✅ |
| 重點摘要（3-5 點） | ✅ | ❌ | ❌ |
| 風險等級標示 | ✅ | 部分 | ✅ |
| **協商對策建議** | ✅ | ❌ | 部分 |
| **MAS 雙重驗證** | ✅ confirmed / pending | ❌ | ❌ |
| **AI 主動表達不確定（Pending）** | ✅ | ❌ | ❌ |
| **可量化驗證（gold set）** | ✅ 100% recall，38 筆 | ❌ | LawGeex 94%（NDA 盲測） |
| 繁體中文 SLA 優化 | ✅ | ❌ | ❌（英文為主） |
| 企業私有雲部署 | ✅ | ✅ | ❌（SaaS 為主） |
