...
1. **機密不可讀**：`.env`（真 API key）與 `pic_contract/`、`nda_contract/`、`sla_contract/`（真實公司合約）**受 deny + hook 雙重保護**。列舉/搜尋（`ls`/`grep`/`git`）放行；但**讀取內容**（`cat`/`head`/`python 開檔`）或用 Read/Edit/Write 工具會被擋。若 Bash 被 `guard_sensitive` hook 擋下（exit 2），那是**刻意的**，**不要重試**，改用不讀取該內容的做法或請 User 處理。詳見 [docs/harness/A_diagnosis.md](docs/harness/A_diagnosis.md)。
2. **不要搬移/改名 `.claude/skills/*.md`**：那 5 個平面檔**不是可用的 Claude Code skill**（`/contract-diff` 等指令不存在），它們是 `src/` 在 runtime 讀取的 **prompt 資產**。**編輯內容可以**；但**搬移、改名、或轉成 `<name>/SKILL.md` 目錄格式會弄壞 4 處產品 loader**——此限制已由 hook + `Write(.claude/skills/*/**)` deny **機制強制**（不是靠自覺）。**動它前必讀** [docs/harness/skills_runtime_assets.md](docs/harness/skills_runtime_assets.md)。
