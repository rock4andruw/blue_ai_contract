...
- [x] **風險規則擴充**：11 → 14 條規則（新增 IP 歸屬、責任反轉、保密範圍、保密單雙向）
- [x] **MAS Phase 1.5**：
...
7. **`candidate_rules.jsonl` 複核**：目前累積 403 筆 Verification Agent 的 Case C 發現（截至 2026-07-03，筆數會持續增加）。跨合約重複分析顯示：
   ...
   - 「新類別候選（未歸類）」跨 5/9 組，內容集中在「刪除續約/銜接條款」「刪除加價服務條款」——現有 14 條規則跟分類法完全沒覆蓋，是真實存在的空白
...
### 一句話定位（2026-07-02 確定，開場優先用這句）

> **「我們是工程化與 MAS 的結合。」**

系統本來就是 multi-agent 架構——Verification Agent（語意補漏）+ MAS 的 Agent A（嚴格）/ Agent B（平衡），三個 LLM 角色各自分工協作。差別在於我們對「哪裡該讓 agent 自主判斷、哪裡不該」做了工程判斷：核心風險判斷維持確定性（規則引擎，法務容不下賭運氣），語意理解、交叉驗證、法律依據檢索這些地方，才交給 agent 處理。

**這句話取代掉之前偏防禦性的講法**（一直強調「我們不是自主 agent」）——改成主動講「我們是什麼」：工程紀律決定「什麼時候不該用 agent」，MAS 決定「什麼時候該用、怎麼協作」，兩者結合才是完整的系統設計判斷力，不是選邊站。同一句話能同時滿足「想聽到 agent/agentic」的聽眾，跟會追問「這真的是 agent 嗎」的技術評審。
...
> 我們是 multi-agent 沒錯——Verification Agent、MAS 的 Agent A/B，三個角色各自獨立判斷再交叉驗證，這是真正的多 agent 協作。但我們刻意沒有讓風險判斷本身變成自主 agentic loop（工具自主呼叫、自己決定要不要重試換方向）——這不是能力不到，是工程判斷：法務審查的核心結論需要可預測、可測試，不能賭一次 agent 自主決策的運氣。所以我們的定位是「工程化 + MAS 的結合」：知道什麼時候該用 agent 協作、什麼時候該守住確定性，這個判斷力本身就是設計的一部分。
